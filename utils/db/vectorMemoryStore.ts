 ...
 */
export const getVectorMemoryHeaders = async (charId: string): Promise<{ id: string; title: string; content: string; emotionalJourney?: string; importance: number; createdAt: number; deprecated?: boolean }[]> => {
    const db = await openDB();
    if (!db.objectStoreNames.contains(STORE_VECTOR_MEMORIES)) return [];
    return new Promise((resolve, reject) => {
        const tx = db.transaction(STORE_VECTOR_MEMORIES, 'readonly');
        ...
        const headers: { id: string; title: string; content: string; emotionalJourney?: string; importance: number; createdAt: number; deprecated?: boolean }[] = [];
        request.onsuccess = () => {
            const cursor = request.result;
            if (cursor) {
                const v = cursor.value;
                headers.push({ id: v.id, title: v.title, content: v.content, emotionalJourney: v.emotionalJourney, importance: v.importance, createdAt: v.createdAt, deprecated: v.deprecated });
                cursor.continue();
            } else {
                resolve(headers);
            }
