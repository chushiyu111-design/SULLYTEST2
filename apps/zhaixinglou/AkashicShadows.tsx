...
const AkashicShadows: React.FC<Props> = ({
    ...
    const handlePressStart = useCallback((e: React.PointerEvent) => {
        ...
        pressTimer.current = setTimeout(() => {
            ...
            // After dissolution animation, transition to dialog
            setTimeout(() => {
                setTimeout(() => {
                    setPhase('dialog');
                    // Add greeting as first message if empty
    ...
    const handleSend = useCallback(async () => {
        ...
        try {
            ...
            const reply = await fetchSecondaryApi(apiConfig, chatMessages, { temperature: 1.0, max_tokens: 800 });
    ...
    if (phase === 'entrance') {
        return (
            ...
            >
                ...
                {/* Floating silver text fragments — bright silver with glow */}
                {floatingTexts.map(t => (
                    <span
                        key={t.id}
                        className={`absolute pointer-events-none select-none transition-all ${t.dissolved ? 'akashic-dissolve' : ''}`}
                        style={{
                            left: `${t.x}%`,
                            top: `${t.y}%`,
                            fontSize: `${t.size}px`,
                            ...
                            textShadow: '0 0 6px rgba(232,236,244,0.7), 0 0 14px rgba(200,210,230,0.5), 0 0 28px rgba(180,195,220,0.3), 0 0 42px rgba(160,180,210,0.15)',
                            animation: t.dissolved ? 'none' : `akashic-float ${t.duration}s ease-in-out ${t.delay}s infinite alternate`,
                            transform: t.dissolved ? 'scale(1.5) translateY(-30px)' : undefined,
                    ...
                    <div className="absolute inset-0 flex flex-col items-center justify-center gap-5 z-10">
                        ...
                        >
                            <img
                                ...
                                alt="旧神符号"
                                className={`w-16 h-16 object-contain transition-all duration-500 ${longPressActive ? 'opacity-90 scale-105' : 'opacity-50'}`}
                                style={{
                ...
                {!isApiConfigured && (
                    <div className="absolute bottom-8 left-0 right-0 flex flex-col items-center gap-3 z-20">
                        ...
                        <button onClick={onOpenSettings} className="px-5 py-2 bg-[#c0c8d8]/10 border border-[#c0c8d8]/30 rounded-xl text-[#c0c8d8] text-xs active:scale-95 transition-transform">
                            前往设置
                        </button>
                    </div>
                )}
        ...
        >
            ...
            <div className="flex-1 relative z-20 flex flex-col items-center justify-center">
                ...
                <div className="relative" style={{ width: '250px', height: '250px' }}>
                    {/* Outer rotating ring of Latin words */}
                    <div className="absolute inset-0" style={{ animation: 'akashic-sigil-rotate 60s linear infinite' }}>
                            ...
                            return (
                                <span
                                    ...
                                    style={{
                                        ...
                                        fontFamily: 'ZhaixinglouFont, serif',
                                        letterSpacing: '0.1em',
                                        textShadow: '0 0 6px rgba(232,236,244,0.4)',
                                        width: '44px',
                                        textAlign: 'center',
                                    }}
                                >
                    ...
                    {/* Inner counter-rotating ring (subtle geometric) */}
                    <div className="absolute inset-[35px] rounded-full border border-[#e8ecf4]/[0.06]" style={{
                        animation: 'akashic-sigil-rotate-reverse 45s linear infinite',
                    }}>
                        {/* Four cardinal dots */}
                        {[0, 90, 180, 270].map(deg => (
                            <div
                                key={deg}
                                className="absolute w-1 h-1 rounded-full bg-[#e8ecf4]/20 left-1/2 top-1/2"
                                style={{ transform: `rotate(${deg}deg) translateY(-50%) translateY(-${(250 - 70) / 2}px)`, marginLeft: '-2px' }}
                            />
                        ))}
                    </div>
